  ...
  return (
    <div className="p-2 max-w-6xl mx-auto w-full">
      <div className="px-2 space-y-8">
        {projectManagementDocs.map((section, index) => (
          <section key={index} className="prose dark:prose-invert max-w-none">
            ...
            {section.collaborationFeatures && (
              <div className="mt-8 p-6 ">
                <h2 className="text-2xl font-semibold text-gray-800 dark:text-gray-200">
                  {section.collaborationFeatures.title}
                </h2>
                <ul className="list-disc pl-6 mt-4 space-y-2 text-gray-700 dark:text-gray-300">
                  {section.collaborationFeatures.items.map((item, idx) => (
                    <li key={idx}>{item}</li>
                  ))}
                </ul>
              </div>
